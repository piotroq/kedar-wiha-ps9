<div class="pos-producttabs-widget display-items-{$classes}" data-ajax="1">
  <ul class="tab-titles nav navtab-products">
    {foreach from=$tab_titles item=tab_title name=posTab}
      <li class="nav-item" data-id="{$tab_title.id}">
        <a class="nav-link {if $smarty.foreach.posTab.index == 0}active{/if}" data-toggle="tab" href="#tab-pane-{$tab_title.id}">{$tab_title.title}</a>
      </li>
    {/foreach}
  </ul>
  <div class="tab-content">
    {foreach from=$tab_contents item=tab_content name=posTab}
      <div class="{if $smarty.foreach.posTab.index == 0}active{/if} tab-pane fade in" id="tab-pane-{$tab_content.id}" data-id="{$tab_content.id}" data-tab_content='{$tab_content.tab_data}'>
        {if $tab_content.products}
          {if $carousel_active}
            {if $specific_layout}
              <div class="tab-specific-layout">
                <div class="tabproduct-first-content">
                  {foreach from=$tab_content.products item="product" name="producttab"}
