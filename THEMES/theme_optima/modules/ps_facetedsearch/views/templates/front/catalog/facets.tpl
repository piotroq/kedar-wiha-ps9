...
{if $displayedFacets|count}
  <div id="search_filters">
    {block name='facets_title'}
      <p class="text-uppercase h6">{l s='Filter By' d='Shop.Theme.Actions'}</p>
    {/block}
    ...
    {foreach from=$displayedFacets item="facet"}
      <section class="facet clearfix">
        ...
        {/foreach}
        <div class="facet-content {if $facet.widgetType == 'slider'}facet-slider-content{/if}">
          {if in_array($facet.widgetType, ['radio', 'checkbox'])}
            {block name='facet_item_other'}
              <ul id="facet_{$_expand_id}" class="facet-{$facet.widgetType}">
                {foreach from=$facet.filters key=filter_key item="filter"}
                  ...
                  <li {if isset($filter.properties.color) || isset($filter.properties.texture)}class="color-filter-type"
                      {/if}{if $filter.type == 'manufacturer'} class="manufacturer-filter" {/if}>
                      {if isset($filter.properties.color) || isset($filter.properties.texture)}
                        <span class="custom-checkbox">
                          ...
                          {/if}
                          <span class="color-filter-tooltip">{$filter.label}</span>
                        </span>
                      {elseif $filter.type == 'manufacturer'}
                        <span class="custom-checkbox">
                          <input id="facet_input_{$_expand_id}_{$filter_key}" data-search-url="{$filter.nextEncodedFacetsURL}"
                            type="checkbox" {if $filter.active }checked{/if}>
                          <img src="{$link->getManufacturerImageLink($filter.value)}" />
                        </span>
                      {else}
