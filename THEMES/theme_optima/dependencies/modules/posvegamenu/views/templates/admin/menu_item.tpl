...
<div class="panel"><h3><i class="icon-list-ul"></i> {l s='Menu Item' mod='posvegamenu'}</h3>
	<div class="form-wrapper" id="menuContent" >
		<div class="form-group">
			<label class="control-label col-lg-3">{l s='Title' mod='posvegamenu'}</label>
			<div class="col-lg-9">
				{foreach from=$languages item=language}
					...
					{/if}
				{/foreach}
				<p class="help-block" style="clear:both;">{l s='>>> If use Prestashop link and want to use Prestashop title, leave it empty.' mod='posvegamenu'}</p>
			</div>
		</div>
		<div class="form-group lab-type-link">
			<label class="control-label col-lg-3">{l s='Type Link' mod='posvegamenu'}</label>
			<div class="col-lg-9">
				<select id="type_link" name="type_link" class="form-control fixed-width-xl" data-default="0">
	                <option value="0" {if $menu->type_link == 0}selected="selected" {/if}>{l s='PrestaShop Link' mod='posvegamenu'}</option>
	                <option value="1" {if $menu->type_link == 1}selected="selected" {/if}>{l s='Custom Link' mod='posvegamenu'}</option>
	                <option value="2" {if $menu->type_link == 2}selected="selected" {/if}>{l s='None' mod='posvegamenu'}</option>
	            </select>
			</div>
		</div>
		
		<div class="form-group custom_link">
			<label class="control-label col-lg-3">{l s='Custom link' mod='posvegamenu'}</label>
			<div class="col-lg-9">
				{foreach from=$languages item=language}
					{if $languages|count > 1}
						<div class="translatable-field lang-{$language.id_lang|intval}" {if $language.id_lang != $id_language}style="display:none"{/if}>
		...
		<div class="form-group ps_link">
			<label class="control-label col-lg-3">{l s='PrestaShop Link' mod='posvegamenu'}</label>
			<div class="col-lg-9">
		...
		<div class="form-group">
			<label class="control-label col-lg-3">{l s='Sub Title' mod='posvegamenu'}</label>
			<div class="col-lg-9">
				{foreach from=$languages item=language}
					{if $languages|count > 1}
						<div class="translatable-field lang-{$language.id_lang|intval}" {if $language.id_lang != $id_language}style="display:none"{/if}>
					{/if}
					<div class="col-lg-6">
					<input type="text" class="subtitle" id="subtitle_{$language.id_lang|intval}" name="subtitle_{$language.id_lang|intval}" value="{if isset($menu->subtitle[$language.id_lang|intval]) && $menu->subtitle[$language.id_lang|intval]}{$menu->subtitle[$language.id_lang|intval]}{/if}"/>
					</div>
					{if $languages|count > 1}
						<div class="col-lg-2">
							<button type="button" class="btn btn-default dropdown-toggle" tabindex="-1" data-toggle="dropdown">
		...
		<div class="form-group lab-type-icon">
			<label class="control-label col-lg-3">{l s='Use icon' mod='posvegamenu'}</label>
			<div class="col-lg-9">
				<select id="type_icon" name="type_icon" class="form-control fixed-width-xl" data-default="0">
	                <option value="0" {if $menu->type_icon == 0}selected="selected" {/if}>{l s='No' mod='posvegamenu'}</option>
	                <option value="1" {if $menu->type_icon == 1}selected="selected" {/if}>{l s='Font-Awesome Icon' mod='posvegamenu'}</option>
	                <option value="2" {if $menu->type_icon == 2}selected="selected" {/if}>{l s='Image Icon' mod='posvegamenu'}</option>
	            </select>
			</div>
		</div>
		
		<div class="form-group lab-fw-icon">
			<label class="control-label col-lg-3">{l s='Font-Awesome Icon' mod='posvegamenu'}</label>
			<div class="col-lg-9">
				<input type="text" class="icon_class fixed-width-xl" id="icon_class" name="icon_class" value="{$menu->icon_class|escape:'html':'UTF-8'}"/>
				<p>{l s='Put class icon of Font-Awesome at :' mod='posvegamenu'} <a href="https://fontawesome.com/v4.7/icons/">https://fontawesome.com/v4.7/icons</a> ex: <span>&lt;i class="</span>fa fa-camera<span>"&gt;&lt;/i&gt;</span></p>   
			</div>
		</div>
		<div class="form-group lab-img-icon">
			<label class="control-label col-lg-3">{l s='Image Icon' mod='posvegamenu'}</label>
			<div class="col-lg-9">
                <div class="col-lg-7">
                    <input type="text" id="icon_img" name="icon_img" value="{if $menu->icon}{$menu->icon}{/if}"/>
                    <a href="filemanager/dialog.php?type=1&field_id=icon_img" class="btn btn-default iframe-column-upload"  data-input-name="icon_img" type="button">{l s='Select image' mod='posvegamenu'} <i class="icon-angle-right"></i></a>
	...
	<div class="form-group">
		<label class="control-label col-lg-1" for="">{l s='Subtitle style' mod='posvegamenu'}</label>
		<div class="col-lg-11">
			<div class="row">
				
				<div class="form-group col-lg-3" style="width: 23%;">
	                <label class="control-label col-lg-4" for="subtitle_color">{l s='Color' mod='posvegamenu'}</label>
	                <div class="col-lg-8">
	            ...
	            <div class="form-group col-lg-3" style="width: 23%;">
	                <label class="control-label col-lg-4" for="subtitle_bgcolor">{l s='Background color' mod='posvegamenu'}</label>
	                <div class="col-lg-8">
	                    <div class="input-group">
	            ...
	            <div class="form-group col-lg-2">
	                <label class="control-label col-lg-6" for="subtitle_fontsize">{l s='Font size' mod='posvegamenu'}</label>
	                <div class="col-lg-6">
	                    <div class="input-group">
	            ...
	            <div class="form-group col-lg-2">
	                <label class="control-label col-lg-6" for="subtitle_lineheight">{l s='Line height' mod='posvegamenu'}</label>
	                <div class="col-lg-6">
	                    <div class="input-group">
	            ...
	            <div class="form-group col-lg-2" style="width: 20%;">
			        <label class="control-label col-lg-6" for="subtitle_transform">{l s='Text transform' mod='posvegamenu'}</label>
			        <div class="col-lg-6">
			            <select id="subtitle_transform" name="subtitle_transform" class="form-control" data-default="0">
			                <option value="0" {if $menu->subtitle_transform == 0}selected="selected" {/if}>{l s='none' mod='posvegamenu'}</option>
			                <option value="1" {if $menu->subtitle_transform == 1}selected="selected" {/if}>{l s='UPPERCASE' mod='posvegamenu'}</option>
			                <option value="2" {if $menu->subtitle_transform == 2}selected="selected" {/if}>{l s='Capitalize' mod='posvegamenu'}</option>
			            </select>
			        </div>
			    </div>
		    </div>
	...
	<div class="form-group" id="item_type_form">
        <label class="control-label col-lg-3" for="submenu_type">{l s='Submenu type' mod='posvegamenu'}</label>
        <div class="col-lg-9">
            <select id="submenu_type" name="submenu_type" class="form-control fixed-width-lg" data-default="1">
                <option value="0" {if $menu->submenu_type == 0}selected="selected" {/if}>{l s='Mega' mod='posvegamenu'}</option>
                <option value="1" {if $menu->submenu_type == 1}selected="selected" {/if}>{l s='Flyout' mod='posvegamenu'}</option>
                <option value="2" {if $menu->submenu_type == 2}selected="selected" {/if}>{l s='None' mod='posvegamenu'}</option>
            </select>
            <p class="help-block">{l s='Flyout only is available when you select category in Type Link' mod='posvegamenu'}</p>
        </div>
    </div>
	<div class="form-group">
		<label class="control-label col-lg-3">{l s='Add specific class' mod='posvegamenu'}</label>
		<div class="col-lg-9">
	...
	<div class="form-group">
		<label class="control-label col-lg-3">{l s='New window' mod='posvegamenu'}</label>
		<div class="col-lg-9">
	...
	<div class="form-group">
		<label class="control-label col-lg-3">{l s='Active' mod='posvegamenu'}</label>
		<div class="col-lg-9">
			<span class="switch prestashop-switch fixed-width-lg">
	...
	<div class="panel-footer">
		<input type="hidden" name="id_posvegamenu_item" id="id_posvegamenu_item" value="{if isset($menu->id)}{$menu->id|intval}{/if}"/>
		<button type="submit" value="1" id="module_form_submit_btn" name="submitMenuItem" class="btn btn-default pull-right">
			<i class="process-icon-save"></i> Save
		</button>
		<a href="index.php?controller=AdminModules&amp;configure=posvegamenu&amp;token={$token|escape:'html':'UTF-8'}" class="btn btn-default">
