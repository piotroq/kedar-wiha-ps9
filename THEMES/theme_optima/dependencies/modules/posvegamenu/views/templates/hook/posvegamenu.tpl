...
<div class="pos-menu-vertical behaviour-{$menu_config.posvegamenu_behaviour}" data-more-less="{$menu_config.posvegamenu_more_less}">
	<h3 class="title_vertical hidden-md-down"><i class="icon-rt-bars-solid"></i>{$menu_config.posvegamenu_title} </h3>
	<ul class="menu-content"> 
		{foreach from=$menus item=menu name=menus}	 			
				<li class="{if $menu.link == {$urls.pages.index} && $page.page_name == 'index'}home{/if} menu-item menu-item{$menu.id_posvegamenu_item} {$menu.item_class|escape:'html':'UTF-8'} {if $menu.submenu_type == 0 && count($menu.sub_menu) > 0}hasChild{/if} {if isset($menu.selected_item) && $menu.selected_item == 1}active{/if}">
					...
					<a  style="{if $menu.type_link==2}cursor: auto;{/if}" href="{if $menu.type_link==0}{$menu.link|escape:'html':'UTF-8'}{elseif $menu.type_link ==1}{$menu.custom_link|escape:'html':'UTF-8'}{else}javascript:void(0){/if}" {if $menu.new_window == 1} target="_blank" {/if}>
						...
						{if $menu.submenu_type == 0 && count($menu.sub_menu) > 0} <i class="hidden-md-down icon_sub icon-rt-arrow-right"></i>{/if}
						{if $menu.submenu_type == 1 && $menu.sub_menu} <i class="hidden-md-down icon_sub icon-rt-arrow-right"></i>{/if} 
					</a>
					{if $menu_config.posvegamenu_sub_animation == 1} 
						{assign var='animation_class' value='menu_noanimation'}
					{elseif $menu_config.posvegamenu_sub_animation == 2}
								...
								<div class="pos-menu-row row {$menu.item_class|escape:'html':'UTF-8'}">
									{if isset($menu_row.list_col) && count($menu_row.list_col) > 0}
										{foreach from=$menu_row.list_col item= menu_col name=menu_col}
											<div class="pos-menu-col {if $menu_col.width > 3}col-xs-12{else}col-xs-6{/if} col-lg-{$menu_col.width|escape:'html':'UTF-8'} {$menu_col.class|escape:'html':'UTF-8'} {if !$menu_col.active_mobile}hidden-mobile{/if}">
												{if $menu_col.title}
