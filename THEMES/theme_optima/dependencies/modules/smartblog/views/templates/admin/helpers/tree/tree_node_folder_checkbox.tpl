...
{*		{if isset($node['id_category']) && $node['id_category'] != $root_category}*}
		...
		<label class="tree-toggler">{if isset($node['name'])}{$node['name']}{/if}{if isset($node['selected_childs']) && (int)$node['selected_childs'] > 0} {l s='(%s selected)' sprintf=$node['selected_childs']}{/if}</label>
	</span>
	<ul class="tree">
		{$children}
	</ul>
