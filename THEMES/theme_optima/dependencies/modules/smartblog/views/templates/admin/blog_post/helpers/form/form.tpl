...
<form id="{if isset($fields.form.form.id_form)}{$fields.form.form.id_form}{else}{if $table == null}configuration_form{else}{$table}_form{/if}{if isset($smarty.capture.table_count) && $smarty.capture.table_count}_{$smarty.capture.table_count|intval}{/if}{/if}" class="defaultForm form-horizontal{if isset($name_controller) && $name_controller} {$name_controller}{/if}"{if isset($current) && $current} action="{$current}{if isset($token) && $token}&amp;token={$token}{/if}"{/if} method="post" enctype="multipart/form-data"{if isset($style)} style="{$style}"{/if} novalidate>
	...
	<div class="panel-heading tab" >
		{l s='Images' d='Modules.Smartblog.Form'}
	...
	<div class="row">
		<div class="form-group">
			<label class="control-label col-lg-3 file_upload_label">
				<span class="label-tooltip" data-toggle="tooltip"
					title="{l s='Format:' d='Modules.Smartblog.Form'} JPG, GIF, PNG. {l s='Filesize:' d='Modules.Smartblog.Form'} {$max_image_size|string_format:"%.2f"} {l s='MB max.' d='Modules.Smartblog.Form'}">
					{if isset($id_image)}{l s='Edit this gallery\'s image:' d='Modules.Smartblog.Form'}{else}{l s='Add a new image to this Gallery' d='Modules.Smartblog.Form'}{/if}
				</span>
			</label>
			<div class="col-lg-9">
				{$image_uploader}
		...
		<div class="form-group">
			<label class="control-label col-lg-3">
				<span class="label-tooltip" data-toggle="tooltip"
					title="{l s='Invalid characters:' d='Modules.Smartblog.Form'} <>;=#{}">
					{l s='Caption' d='Modules.Smartblog.Form'}
				</span>			
			</label>
			<div class="col-lg-9">
	...
	<table class="table tableDnD" id="imageTable">
		<thead>
			<tr class="nodrag nodrop"> 
				<th class="fixed-width-lg"><span class="title_box">{l s='Image' d='Modules.Smartblog.Form'}</span></th>
				<th class="fixed-width-lg"><span class="title_box">{l s='Caption' d='Modules.Smartblog.Form'}</span></th>
				<th class="fixed-width-xs"><span class="title_box">{l s='Position' d='Modules.Smartblog.Form'}</span></th>
		...
		<tr id="image_id">
			...
			<td>
				<a href="#" class="delete_product_image pull-right btn btn-default" >
					<i class="icon-trash"></i> {l s='Delete this image' d='Modules.Smartblog.Form'}
				</a>
			</td>
		</tr>
	</table>
	...
	<script type="text/javascript">
		var upbutton = '{l s='Upload an image' d='Modules.Smartblog.Form'}';
		var come_from = '{$table}';
		var success_add =  '{l s='The image has been successfully added.' d='Modules.Smartblog.Form'}';
		var id_tmp = 0;
		...
		$(document).ready(function(){
			...
			$('.delete_product_image').die().live('click', function(e)
			{
				...
				if (confirm("{/literal}{l s='Are you sure?' d='Modules.Smartblog.Form' js=1}{literal}"))
				doAdminAjax({
		...
		{else}
	<div class="alert alert-warning">  {l s='You must save this post before adding images.' d='Modules.Smartblog.Form'}  </div>
				...
				{if $key == 'legend'}
					...
					{block name="legend"}
						<div class="panel-heading">
							{if isset($field.image) && isset($field.title)}<img src="{$field.image}" alt="{$field.title}" />{/if}
					...
					{foreach $field as $input}
						...
						{else}
							...
							{block name="field"}
										...
										{if $input.type == 'tags'}
											...
											<script type="text/javascript">
												$().ready(function () {
													...
													$('#'+input_id).tagify({delimiters: [13,44], addTagPrompt: '{/literal}{l s='Add tag' d='Modules.Smartblog.Form'}{literal}'});
													$({/literal}'#{$table}{literal}_form').submit( function() {
														$(this).find('#'+input_id).val($('#'+input_id).tagify('serialize'));
													});
												});
								...
								{elseif $input.type == 'blog_post_type'}
									...
									{foreach $input.values as $value}
										<div class="radio {if isset($input.class)}{$input.class}{/if}">
											{strip}
											<label>
											<input type="radio"	name="{$input.name}" id="{$value.id}" value="{$value.value}"{if $fields_value[$input.name] == $value.value} checked="checked"{/if}{if isset($input.disabled) && $input.disabled} disabled="disabled"{/if}/>
								...
								<input type="hidden" name="nameAccessories" id="nameAccessories" value="{foreach from=$accessories item=accessory}{$accessory.name}¤{/foreach}" />
				...
				excludeIds:getAccessoriesIds()
								...
								{elseif $input.type == 'select'}
									...
									{else}
										<select name="{$input.name}"
												class="{if isset($input.class)}{$input.class}{/if} fixed-width-xl"
												id="{if isset($input.id)}{$input.id}{else}{$input.name}{/if}"
								...
								{elseif $input.type == 'switch'}
									<span class="switch prestashop-switch fixed-width-lg">
										...
										<label {if $value.value == 1} for="{$input.name}_on"{else} for="{$input.name}_off"{/if}>
											{if $value.value == 1}
												{l s='Yes' d='Modules.Smartblog.Form'}
											{else}
												{l s='No' d='Modules.Smartblog.Form'}
											{/if}
										</label>
										{/strip}
										{/foreach}
								...
								{elseif $input.type == 'textarea'}
									...
									<div class="form-group translatable-field lang-{$language.id_lang}"{if $language.id_lang != $defaultFormLanguage} style="display:none;"{/if}>
										<div class="col-lg-9">
									{/if}
											<textarea name="{$input.name}_{$language.id_lang}" class="{if isset($input.autoload_rte) && $input.autoload_rte}rte autoload_rte{if isset($input.class)} {$input.class}{/if}{else}{if isset($input.class)} {$input.class}{else} textarea-autosize{/if}{/if}">{$fields_value[$input.name][$language.id_lang]}</textarea>
									{if $languages|count > 1}	
										</div>
										<div class="col-lg-2">
											<button type="button" class="btn btn-default dropdown-toggle" tabindex="-1" data-toggle="dropdown">
										...
										<div class="checkbox{if isset($input.expand) && strtolower($input.expand.default) == 'show'} hidden{/if}">
											{strip}
												<label for="{$id_checkbox}">
													<input type="checkbox" name="{$id_checkbox}" id="{$id_checkbox}" class="{if isset($input.class)}{$input.class}{/if}"{if isset($value.val)} value="{$value.val}"{/if}{if isset($fields_value[$id_checkbox]) && $fields_value[$id_checkbox]} checked="checked"{/if} />
									...
									<div class="row">
										<div class="col-lg-12">
											<button type="button" id="{$input.name}-btn-change" class="btn btn-default">
												<i class="icon-lock"></i>
												{l s='Change password...' d='Modules.Smartblog.Form'}
											</button>
											<div id="{$input.name}-change-container" class="form-password-change well hide">
												<div class="form-group">
													<label for="old_passwd" class="control-label col-lg-2 required">
														{l s='Current password' d='Modules.Smartblog.Form'}
													</label>
													<div class="col-lg-10">
												...
												<div class="form-group">
													<label for="{$input.name}" class="required control-label col-lg-2">
														<span class="label-tooltip" data-toggle="tooltip" data-html="true" title="" data-original-title="Password should be at least 8 characters long.">						
															{l s='New password' d='Modules.Smartblog.Form'}
														</span>
													</label>
													<div class="col-lg-9">
														<div class="input-group fixed-width-lg">
												...
												<div class="form-group">
													<label for="{$input.name}2" class="required control-label col-lg-2">
														{l s='Confirm password' d='Modules.Smartblog.Form'}
													</label>
													<div class="col-lg-4">
														<div class="input-group fixed-width-lg">
															<span class="input-group-addon">
												...
												<div class="form-group">
													<div class="col-lg-10 col-lg-offset-2">
														...
														<button type="button" id="{$input.name}-generate-btn" class="btn btn-default">
															...
															{l s='Generate password' d='Modules.Smartblog.Form'}
														</button>
													</div>
												</div>
												<div class="form-group">
													<div class="col-lg-10 col-lg-offset-2">
														<p class="checkbox">
															<label for="{$input.name}-checkbox-mail">
																...
																{l s='Send me this new password by Email' d='Modules.Smartblog.Form'}
															</label>
														</p>
													</div>
												</div>
												<div class="row">
													<div class="col-lg-12">
														<button type="button" id="{$input.name}-cancel-btn" class="btn btn-default">
															...
															{l s='Cancel' d='Modules.Smartblog.Form'}
														</button>
													</div>
												</div>
											</div>
										...
										<select name="{$key}" class="fixed-width-lg{if isset($input.class)} {$input.class}{/if}">
											...
											{if $key == 'months'}
												{*
													...
													{l s='January' d='Modules.Smartblog.Form'}
													{l s='February' d='Modules.Smartblog.Form'}
													{l s='March' d='Modules.Smartblog.Form'}
													{l s='April' d='Modules.Smartblog.Form'}
													{l s='May' d='Modules.Smartblog.Form'}
													{l s='June' d='Modules.Smartblog.Form'}
													{l s='July' d='Modules.Smartblog.Form'}
													{l s='August' d='Modules.Smartblog.Form'}
													{l s='September' d='Modules.Smartblog.Form'}
													{l s='October' d='Modules.Smartblog.Form'}
													{l s='November' d='Modules.Smartblog.Form'}
													{l s='December' d='Modules.Smartblog.Form'}
												*}
									...
									{/if}
									 ...
									 {if $show_thumbnail}
										...
										{assign var='max_files' value=1}
										 <div class="form-group">
											<div class="col-lg-12" id="{$id}-images-thumbnails">
												...
												<div>
													{$input.image}
													{if isset($input.size)}<p>{l s='File size' d='Modules.Smartblog.Form'} {$input.size}kb</p>{/if}
													 
													{if isset($input.delete_url)}
													<p>
														<a class="btn btn-default" href="{$input.delete_url}">
															<i class="icon-trash"></i> {l s='Delete' d='Modules.Smartblog.Form'}
														</a>
													</p>
													{/if}
												</div>
										...
										{assign var='max_files' value=1}
											...
											<div class="form-group">
												<div class="col-sm-6">
													<input id="{$id}" type="file" name="{$input.name}{if isset ($multiple) && $multiple}[]{/if}"{if isset($multiple) && $multiple} multiple="multiple"{/if} class="hide" />
													<div class="dummyfile input-group">
														...
														<span class="input-group-btn">
															<button id="{$id}-selectbutton" type="button" name="submitAddAttachments" class="btn btn-default">
																<i class="icon-folder-open"></i> {if isset($multiple) && $multiple}{l s='Add files' d='Modules.Smartblog.Form'}{else}{l s='Add file' d='Modules.Smartblog.Form'}{/if}
															</button>
															{if (!isset($multiple) || !$multiple) && isset($files) && $files|count == 1 && isset($files[0].download_url)}
																<a href="{$files[0].download_url}" class="btn btn-default">
																	<i class="icon-cloud-download"></i>
																	{if isset($size)}{l s='Download current file (%skb)' sprintf=$size d='Modules.Smartblog.Form'}{else}{l s='Download current file' d='Modules.Smartblog.Form'}{/if}
																</a>
															{/if}
														</span>
													</div>
													...
													if (typeof {$id}_max_files !== 'undefined')
													{
														$('#{$id}').closest('form').on('submit', function(e) {
															if ($('#{$id}')[0].files.length > {$id}_max_files) {
																e.preventDefault();
																alert('{l s='You can upload a maximum of %s files'|sprintf:$max_files d='Modules.Smartblog.Form'}');
															}
														});
													}
												});
				...
				{if isset($fieldset['form']['submit']) || isset($fieldset['form']['buttons'])}
					<div class="panel-footer">
						...
						<a href="{$back_url}" class="btn btn-default" onclick="window.history.back();">
							<i class="process-icon-cancel"></i> {l s='Cancel' d='Modules.Smartblog.Form'}
						</a>
						{/if}
	...
	<script type="text/javascript">
		...
		$(document).ready(function() {
			...
			$('.datetimepicker').datetimepicker({
				...
				currentText: '{l s='Now' d='Modules.Smartblog.Form'}',
				closeText: '{l s='Done' d='Modules.Smartblog.Form'}',
				ampm: false,
				amNames: ['AM', 'A'],
				pmNames: ['PM', 'P'],
				timeFormat: 'hh:mm:ss tt',
				timeSuffix: '',
				timeOnlyTitle: '{l s='Choose Time' js=1 d='Modules.Smartblog.Form'}',
				timeText: '{l s='Time' js=1 d='Modules.Smartblog.Form'}',
				hourText: '{l s='Hour' js=1 d='Modules.Smartblog.Form'}',
				minuteText: '{l s='Minute' js=1 d='Modules.Smartblog.Form'}',
			});
