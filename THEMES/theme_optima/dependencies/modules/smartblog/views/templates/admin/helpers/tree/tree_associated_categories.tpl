...
<script type="text/javascript">
	...
	{if isset($use_search) && $use_search == true}
		$('#{$id}-categories-search').bind('typeahead:selected', function(obj, datum){
			var match = $('#{$id}').find(':input[value="'+datum.id_category+'"]').first();
			if (match.length)
			{
				match.each(function(){
						$(this).prop("checked", true);
			...
			{                    
				var selected = [];
				that = this;
				$('#{$id}').find('.tree-selected input').each(
					function()
					{
				...
				{/literal}
						$('#{$id}').html(content);
						$('#{$id}').smartcattree('init');
						$('#{$id}').find(':input[value="'+datum.id_category+'"]').each(function(){
								$(this).prop("checked", true);
								$(this).parent().addClass("tree-selected");
								$(this).parents('ul.tree').each(function(){
									$(this).show();
	...
	$(document).ready(function(){
		...
		$('#{$id}').smartcattree('collapseAll');
		$('#{$id}').find(':input[type=radio]').click(SmarttreeClickFunc);
