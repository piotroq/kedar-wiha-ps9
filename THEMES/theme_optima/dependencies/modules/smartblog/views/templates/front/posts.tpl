		...
		<nav class="breadcrumb smart-blog-breadcrumb">
		  <ol>
		      ...
		      <li>
		        ...
		        <span>{l s='All Post'  d='Modules.Smartblog.Posts'}</span>
		        </a>
		      </li>
		      {if $title_category != ''}
...
{block name='page_content'}
		{capture name=path}<a href="{smartblog::GetSmartBlogLink('module-smartblog-list')|escape:'htmlall':'UTF-8'}">{l s='All Blog News'  d='Modules.Smartblog.Posts'}</a><span class="navigation-pipe"></span>{$meta_title|escape:'htmlall':'UTF-8'}{/capture}
				...
				<div class="smart-blog-posts-header-area">
					...
					<div class="sdsarticleHeader">
						<span class="smart-blog-posts-info">
							{if $smartshowauthor ==1} {l s='Posted by '  d='Modules.Smartblog.Posts'} &nbsp;<i class="icon-rt-person-circle-outline"></i>
							<span itemprop="author">{if $smartshowauthorstyle != 0}{$firstname} {$lastname}{else}{$lastname} {$firstname}{/if}</span>{/if}&nbsp;
							<i class="icon-rt-time-outline"></i>&nbsp;<span itemprop="dateCreated">{$created|escape:'htmlall':'UTF-8'}</span>
							<span itemprop="articleSection">
								{$assocCats = BlogCategory::getPostCategoriesFull($post.id_post)}
								{$catCounts = 0}
								{if !empty($assocCats)}
									&nbsp;&nbsp;<i class="icon-rt-Agenda"></i>&nbsp;
				...
				<div itemprop="articleBody">
	            	...
	            	{if $tags != ''}
		                <div class="sdstags-update smart-blog-post-tags">
		                    <span class="tags"><span>{l s='Tags:'  d='Modules.Smartblog.Posts'} </span> 
			...
			<ul class="footer_links smart-blog-posts-navigation">
				{foreach from=$posts_previous item="post"}
        			{if isset($post.id_smart_blog_post)}
						<li>
							<a title="{l s='Prevoius Post'  d='Modules.Smartblog.Posts'}" href="{$smartbloglink->getSmartBlogPostLink($post.id_smart_blog_post,$post.link_rewrite)|escape:'htmlall':'UTF-8'}" class="btn-primary"><span>{l s='Prev Post'  d='Modules.Smartblog.Posts'}</span></a>
						</li>
					{/if}
				{/foreach}
				{foreach from=$posts_next item="post"}
					{if isset($post.id_smart_blog_post)}
						<li class="pull-right">
							<a title="{l s='Next Post'  d='Modules.Smartblog.Posts'}" href="{$smartbloglink->getSmartBlogPostLink($post.id_smart_blog_post,$post.link_rewrite)|escape:'htmlall':'UTF-8'}" class="btn-primary"><span>{l s='Next Post'  d='Modules.Smartblog.Posts'} <i class="icon-chevron-right"></i> </span></a>
						</li>
					{/if}
				{/foreach}
			</ul>
		...
		{if $countcomment != ''}
			<div id="articleComments" class="smart-blog-post-comments-view-area">
        		<h3 class="smart-blog-comments-title">{if $countcomment != ''}{$countcomment|escape:'htmlall':'UTF-8'}{else}{l s='0'  d='Modules.Smartblog.Posts'}{/if}{l s=' Comments'  d='Modules.Smartblog.Posts'}<span></span></h3>
   				 <div id="comments">      
			...
			{if Configuration::get('smartenablecomment') == 1}
				{if $comment_status == 1}
					<div class="smartblogcomments smart-blog-commentform-area" id="respond">
				    	<h4 class="comment-reply-title" id="reply-title">{l s='Leave a Reply'   d='Modules.Smartblog.Posts'}
						...
						<div id="commentInput" class="smart-blog-commentform-input">
							<form action="" method="post" id="commentform">
										...
										{else}
											<div class="smart-blog-comm-row">
												<div class="smart-blog-single-comm smart-blog-pr-10">
													<input type="text" tabindex="1" class="inputName form-control" placeholder="{l s='Name*'   d='Modules.Smartblog.Posts'}" value="" name="name">
												</div>
												<div class="smart-blog-single-comm smart-blog-pl-10">
													<input type="text" tabindex="2" class="inputMail form-control" value="" placeholder="{l s='Email*'  d='Modules.Smartblog.Posts'}" name="mail">
												</div>
											</div>
											<div class="smart-blog-comm-row">
												<div class="smart-blog-single-comm smart-blog-single-comm-full-width"><input type="text" tabindex="3" value="" name="website" placeholder="{l s='Website'   d='Modules.Smartblog.Posts'}" class="form-control"></div>
											</div>
										{/if}	
										<div class="smart-blog-comm-row">
											<div class="smart-blog-single-comm smart-blog-single-comm-full-width"><textarea tabindex="4" placeholder="{l s='Comment'   d='Modules.Smartblog.Posts'}" class="inputContent form-control" rows="8" cols="50" name="comment"></textarea></div>
										</div>
										{if Configuration::get('smartcaptchaoption') == '1'}
											<div class="smart-blog-comm-row">
												<div class="smart-blog-single-comm"><img src="{$captcha_url}?width=100&height=40&characters=5"></div>
											</div>
											<div class="smart-blog-comm-row">
												<div class="smart-blog-single-comm"><input type="text" tabindex="" value="" placeholder="{l s='Type Code'  d='Modules.Smartblog.Posts'}" name="smartblogcaptcha" class="smartblogcaptcha form-control"></div>
											</div>
										{/if}
									</div>
								...
								<div class="smart-blog-comm-button-area">
							        <div class="submit">
										<button type="submit" name="addComment" id="submitComment" class="bbutton btn btn-primary" >{l s='Submit'  d='Modules.Smartblog.Posts'}</button>
									</div>
								</div>
    						</form>
						</div>
