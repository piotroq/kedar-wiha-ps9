...
class AdminBlogCategoryController extends AdminController {
	...
	public function l123( $string, $class = null, $addslashes = false, $htmlentities = true ) {
			...
			if ( $error = ImageManager::validateUpload( $_FILES['category_image'], 4000000 ) ) {
				...
				if ( ! move_uploaded_file( $_FILES['category_image']['tmp_name'], $path ) ) {
					...
					foreach ( $images_types as $image_type ) {
						ImageManager::resize(
							$path,
							_PS_MODULE_DIR_ . 'smartblog/images/category/' . $id . '-' . Tools::stripslashes( $image_type['type_name'] ) . '.jpg',
