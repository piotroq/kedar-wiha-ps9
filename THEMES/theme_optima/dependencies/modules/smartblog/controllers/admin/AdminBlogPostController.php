...
class AdminBlogPostController extends ModuleAdminController
{
	...
	public function renderForm()
	{
		...
		if (Hook::getIdByName('displayBackOfficeSmartBlogPosts')) {
			$fileds_backoffice_post = (Hook::exec('displayBackOfficeSmartBlogPosts',array(),'',true));
			foreach ($fileds_backoffice_post as $value_fileds_backoffice_post) {
				$valfields = unserialize($value_fileds_backoffice_post);
				if(isset($valfields['custom_fields_hook'])){
					foreach($valfields['custom_fields_hook'] as $fields){
						$this->fields_form['input'][count($this->fields_form['input'])] = $fields;
					}
				}else{
					$this->fields_form['input'][count($this->fields_form['input'])] = $valfields;
				}
			}
		}
		...
		$this->fields_value['relatedcategorys'] = '';
		$this->fields_value['relatedcategorysmin'] = '';
		$this->fields_value['relatedcategorysmax'] = '';
		...
		if (Tools::isSubmit('submitAddsmart_blog_post')) {
			if (Tools::getValue('id_smart_blog_post')) {
				...
				if (Tools::getValue('associations') && Module::isInstalled('smartblogrelatedproducts')) {
					$res = Db::getInstance()->executeS('SELECT `related_poroduct_id` FROM `' . _DB_PREFIX_ . 'smart_blog_post_related`  WHERE `id_smart_blog_post` = ' . (int) $id_post);
					if (empty($res)) {
						Db::getInstance()->execute(' INSERT INTO `' . _DB_PREFIX_ . 'smart_blog_post_related` (`id_smart_blog_post`, `related_poroduct_id`)VALUES("' . (int) $id_post . '","' . Tools::getValue('associations') . '" )');
					} else {
	...
	public function processDelete()
	{
		if (Validate::isLoadedObject($object = $this->loadObject())) {
			...
			$this->errors[] = $this->l('An error occurred while deleting the object.', [], 'Admin.Notifications.Error') .
				' <b>' . $this->table . '</b> ' .
				$this->l('(cannot load object)', [], 'Admin.Notifications.Error');
		}
	}
}