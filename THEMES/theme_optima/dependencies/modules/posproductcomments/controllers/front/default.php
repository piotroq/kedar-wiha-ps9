...
// Include Module
include_once(dirname(__FILE__).'/../../posproductcomments.php');
// Include Models
include_once(dirname(__FILE__).'/../../PosProductComment.php');
include_once(dirname(__FILE__).'/../../PosProductCommentCriterion.php');

class PosProductCommentsDefaultModuleFrontController extends ModuleFrontController
{
	...
	{
		$module_instance = new PosProductComments();
		...
		if (!$this->context->customer->id && !Configuration::get('POS_PRODUCT_COMMENTS_ALLOW_GUESTS'))
		...
		if (!count($errors))
		{
			$customer_comment = PosProductComment::getByCustomer(Tools::getValue('id_product'), $id_customer, true, $id_guest);
			if (!$customer_comment || ($customer_comment && (strtotime($customer_comment['date_add']) + (int)Configuration::get('POS_PRODUCT_COMMENTS_MINIMAL_TIME')) < time()))
			{

				$comment = new PosProductComment();
				...
				{
					$grade_sum += $grade;
					$product_comment_criterion = new PosProductCommentCriterion($id_product_comment_criterion);
				...
				$result = true;
				Tools::clearCache(Context::getContext()->smarty, $this->getTemplatePath('posproductcomments-reviews.tpl'));
			}
			else
			{
				$result = false;
				$errors[] = $module_instance->l('Please wait before posting another comment').' '.Configuration::get('POS_PRODUCT_COMMENTS_MINIMAL_TIME').' '.$module_instance->l('seconds before posting a new comment');
			}
		}
		else
			$result = false;
	...
	protected function ajaxProcessReportAbuse()
	{
		...
		if (PosProductComment::isAlreadyReport(Tools::getValue('id_product_comment'), $this->context->cookie->id_customer))
			die('0');

		if (PosProductComment::reportComment((int)Tools::getValue('id_product_comment'), $this->context->cookie->id_customer))
			die('1');
	...
	protected function ajaxProcessCommentIsUsefull()
	{
		...
		if (PosProductComment::isAlreadyUsefulness(Tools::getValue('id_product_comment'), $this->context->cookie->id_customer))
			die('0');

		if (PosProductComment::setCommentUsefulness((int)Tools::getValue('id_product_comment'), (bool)Tools::getValue('value'), $this->context->cookie->id_customer))
			die('1');
