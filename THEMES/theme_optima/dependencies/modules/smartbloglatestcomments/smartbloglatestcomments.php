...
class SmartblogLatestComments extends Module
{
    ...
    {
        if (Module::isInstalled(trim('smartblog'))) {
            if (!Module::isEnabled(trim('smartblog'))) {
                return;
            }
        } else {
            return;
    ...
    public function getContent()
    {
        ...
        if (Tools::isSubmit('save' . $this->name)) {
            ...
            if (Module::isInstalled(trim('smartblog'))) {
                ...
                if (!Module::isEnabled(trim('smartblog'))) {
                    return $this->displayError($this->l('This Is A Child Module of SmartBlog. Please Enable SmartBlog to Use This Module'));
                }
            } else {
                return $this->displayError($this->l('This Is A Child Module of SmartBlog. Please Install SmartBlog to Use This Module'));
            }
            $helper = $this->SettingForm();
            $html .= $helper->generateForm($this->fields_form);
            return $html;
