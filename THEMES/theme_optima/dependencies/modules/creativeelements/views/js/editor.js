...
module.exports = Marionette.Behavior.extend( {
	...
	onInsertButtonClick: function() {
		...
		if ( 'insert' === action ) {
			...
			open( 'https://addons.prestashop.com/administrative-tools/44064-creative-elements-elementor-based-pagebuilder.html', '_blank' );
		}
	}
} );
...
App = Marionette.Application.extend( {
	...
	initPreview: function() {
		...
		if ( ! Backbone.$( '#' + previewIframeId ).length ) {
			...
			previewIFrame.src = this.config.preview_link + '&_=' + Date.now();
	...
	initFrontend: function() {
		ceFrontend.setScopeWindow( this.$preview[0].contentWindow );

		ceFrontend.init();

		ceFrontend.elementsHandler.initHandlers();
	},
	...
	onPreviewLoaded: function() {
		...
		this.hotKeys.bindListener( Backbone.$( ceFrontend.getScopeWindow() ) );
		...
		_.defer( function() {
			ceFrontend.getScopeWindow().jQuery.holdReady( false );
		} );
	...
	reloadPreview: function() {
		...
		elementor.$previewContents.find( '.elementor .slick-initialized' ).slick( 'unslick' );
		// Reset Enqueued Fonts
		elementor.helpers._enqueuedFonts = [];
	...
	logSite: function() {
		...
		if ( -1 !== navigator.userAgent.search( 'Firefox' ) ) {
			...
			style = [
				...
				'line-height: 0',
				'font-size: 0',
				'background: url("' + location.origin + elementor.config.assets_url + '../logo.png") center no-repeat',
...
ElementModel = Backbone.Model.extend( {
	...
	createRemoteRenderRequest: function() {
		...
		return Backbone.$.post( elementor.config.preview_link, {
			render: 'widget',
	...
	this.compare = function( leftValue, rightValue, operator ) {
		switch ( operator ) {
			...
			case '!in':
				return -1 === rightValue.indexOf( leftValue );
			case 'contains':
				return leftValue && -1 !== leftValue.indexOf( rightValue );
			case '!contains':
				return !leftValue || -1 === leftValue.indexOf( rightValue );
			case '<':
				return leftValue < rightValue;
...
ControlsCSSParser = ViewModule.extend( {
	...
	getStyleControlValue: function( control, values ) {
		...
		if (control.name.endsWith('_image') && value && value.url) {
			value = {
...
helpers = {
	...
	isActiveControl: function( controlModel, values ) {
		...
		if ( _.isFunction( controlModel.get ) ) {
			...
			// fix for conditions
			var conditions = controlModel.get( 'conditions' );
		} else {
...
module.exports = ViewModule.extend( {
	...
	changeCallbacks: {
		post_title: function( newValue ) {
			var $title = ceFrontend.getElements( '$document' ).find( elementor.config.page_title_selector );
...
BaseElementView = Marionette.CompositeView.extend( {
	...
	runReadyTrigger: function() {
		_.defer( _.bind( function() {
			ceFrontend.elementsHandler.runReadyTrigger( this.$el );
		}, this ) );
	},
...
WidgetView = BaseElementView.extend( {
	...
	attachElContent: function( html ) {
		...
		_.defer( function() {
			ceFrontend.getScopeWindow().jQuery( el ).html( htmlContent );
		} );
	...
	onRender: function() {
		...
		_.defer( function() {
			//Tungxu
			if(editModel.get( 'widgetType' ) == 'pos_banner' || editModel.get( 'widgetType' ) == 'image') return;
