...
class AdminPosProductExtraTabsController extends ModuleAdminController {
	/**
	 * Countries_array
	 *
	 * @var array
	 ...
	 * @var string
	 */
	protected $position_identifier = 'id_posproductextratabs';
	/**
	 * Asso_type
	 *
	 * @var string
	...
	public function __construct() {
		...
		$this->table            = 'posproductextratabs';
		$this->className        = 'Pospextratab';
		$this->lang             = true;
		$this->deleted          = false;
		$this->module           = 'posproductextratab';
		...
		include_once POS_PEXTRATAB_CLASS_DIR . 'pospetab_ajax.php';
		include_once POS_PEXTRATAB_CLASS_DIR . 'pospetab_provider.php';

		$this->fields_list = array(
			'id_posproductextratabs' => array(
				'title'   => $this->l( 'Id' ),
				'width'   => 100,
				'type'    => 'text',
				'orderby' => false,
	...
	public function init() {
		parent::init();
		$this->_join            = 'LEFT JOIN ' . _DB_PREFIX_ . 'posproductextratabs_shop extrabsshops ON a.id_posproductextratabs=extrabsshops.id_posproductextratabs && extrabsshops.id_shop IN(' . implode( ',', Shop::getContextListShopID() ) . ')';
		$this->_select          = 'extrabsshops.id_shop';
		$this->_defaultOrderBy  = 'a.position';
		$this->_defaultOrderWay = 'DESC';
		if ( Shop::isFeatureActive() && Shop::getContext() != Shop::CONTEXT_SHOP ) {
			$this->_group = 'GROUP BY a.id_posproductextratabs';
		}
	...
	public function setMedia( $isNewTheme = false ) {
		...
		$this->addJs( POS_PEXTRATAB_ASSETS_DIR . '/js/admin.js' );
		Media::addJsDef( array( 'pospetab_ajaxurl' => $this->context->link->getAdminLink( 'AdminPosProductExtraTabs' ) ) );
	...
	public function renderForm() {
		...
		if ( Tools::getvalue( 'id_posproductextratabs' ) ) {
			$classypextra_is_edit         = true;
			$posproductextratabs          = new Pospextratab( Tools::getvalue( 'id_posproductextratabs' ) );
			$condition                    = $posproductextratabs->condition;
			$specific_prd_values          = $posproductextratabs->specific_product;
			$specific_product_catg_values = $posproductextratabs->specific_product_catg;
			$specific_product_manu_values = $posproductextratabs->specific_product_manu;

			$provider_obj        = new PospetabProvider();
		...
		$this->fields_form = array(
			'legend'  => array(
				'title' => $this->l( 'Pos Extra Product Tab' ),
			),
			'input'   => array(
				array(
					'type'     => 'text',
				...
				array(
					...
					'name'                => 'title',
					'pospetab_is_edit' => $classypextra_is_edit,
		...
		if ( ! ( $posproductextratabs = $this->loadObject( true ) ) ) {
			return;
		}
		...
		if ( ! Tools::getvalue( 'id_posproductextratabs' ) ) {
			...
			$posproductextratabs = new Pospextratab( Tools::getvalue( 'id_posproductextratabs' ) );
			$this->fields_value['specific_product_temp']      = $posproductextratabs->specific_product;
			$this->fields_value['specific_product_catg_temp'] = $posproductextratabs->specific_product_catg;
			$this->fields_value['specific_product_manu_temp'] = $posproductextratabs->specific_product_manu;
			$this->fields_value['specific_product_condition'] = $posproductextratabs->condition;
		}
	...
	public function initContent() {
		...
		if ( Tools::getvalue( 'pospetab_ajaxgetproducts' ) ) {
			$ajax_obj = new PospetabAjax();
			echo $ajax_obj->getProductsByName();
			die();
		}
		if ( Tools::getvalue( 'pospetab_ajaxgetcats' ) ) {
			$ajax_obj = new PospetabAjax();
			echo $ajax_obj->getCatsByName();
			die();
		}
		if ( Tools::getvalue( 'pospetab_ajaxgetmanus' ) ) {
			$ajax_obj = new PospetabAjax();
	...
	public function processPosition() {
		...
		} elseif ( ! Validate::isLoadedObject( $object = new Pospextratab( (int) Tools::getValue( $this->identifier, Tools::getValue( 'id_posproductextratabs', 1 ) ) ) ) ) {
		...
		if ( ! $object->updatePosition( (int) Tools::getValue( 'way' ), (int) Tools::getValue( 'position' ) ) ) {
			...
			Tools::redirectAdmin( self::$currentIndex . '&' . $this->table . 'Orderby=position&' . $this->table . 'Orderway=asc&conf=5' . ( ( $id_posproductextratabs = (int) Tools::getValue( $this->identifier ) ) ? ( '&' . $this->identifier . '=' . $id_posproductextratabs ) : '' ) . '&token=' . Tools::getAdminTokenLite( 'Admintabcreation' ) );
		}
	}
	...
	public function ajaxProcessUpdatePositions() {
		$id_posproductextratabs = (int) ( Tools::getValue( 'id' ) );
		$way                       = (int) ( Tools::getValue( 'way' ) );
		$positions                 = Tools::getValue( $this->table );
		if ( is_array( $positions ) ) {
			foreach ( $positions as $key => $value ) {
				$pos = explode( '_', $value );
				if ( ( isset( $pos[1] ) && isset( $pos[2] ) ) && ( $pos[2] == $id_posproductextratabs ) ) {
					$position = $key + 1;
					break;
				}
			}
		...
		$posproductextratabs = new Pospextratab( $id_posproductextratabs );
		if ( Validate::isLoadedObject( $posproductextratabs ) ) {
			if ( isset( $position ) && $posproductextratabs->updatePosition( $way, $position ) ) {
				Hook::exec( 'actionposproductextratabsUpdate' );
				die( true );
			} else {
				die( '{"hasError" : true, errors : "Can not update posproductextratabs position"}' );
			}
		} else {
			die( '{"hasError" : true, "errors" : "This posproductextratabs can not be loaded"}' );
		}
	}
	...
	public function processSave() {
		...
		if ( Tools::isSubmit( 'submitAddposproductextratabsAndStay' )
			|| Tools::isSubmit( 'submitAddposproductextratabs' )
		) {
