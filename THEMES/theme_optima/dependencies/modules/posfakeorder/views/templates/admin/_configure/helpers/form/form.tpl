				...
				{elseif $key == 'input'}
					...
					{foreach $field as $input}
						...
						{elseif $input.type == 'infoheading'}
							<div class="infoheading_class col-sm-12 {if isset($input.colorclass)}{$input.colorclass}{/if} " data-name="{$input.name}" id="{$input.name}" style=" background: #eee;">
								<h4 class="infoheading_label col-lg-offset-3" style="font-size:18px;">{$input.label}</h4>
						...
						{else}
							...
							{block name="field"}
								...
								{elseif $input.type == 'vec-switch'}
									<span class="vec-switch vec-switch-{$input.multi} col-lg-9">
										{foreach $input.values as $value}
										<input type="radio" name="{$input.name}" id="{$input.name}_{$value.id}" value="{$value.value}" {if $fields_value[$input.name] == $value.value} checked="checked"{/if}/>
										{strip}
										<label for="{$input.name}_{$value.id}">
											{$value.label}
										</label>
										{/strip}
										{/foreach}
									...
									<style type="text/css">
										.vec-switch{
											-webkit-box-shadow: rgb(0 0 0 / 15%) 0 1px 4px 1px inset;
										    background-color: #eee;
										    border-radius: 3px;
										    box-shadow: inset 0 1px 4px 1px rgb(0 0 0 / 15%);
										    color: #333;
										    margin-top: 3px;
										    text-align: center;
										    display: block;
										...
										.vec-switch input:checked+label {
										    color: #fff;
										}
										.vec-switch input:last-of-type:checked~a {
								...
								{else if $input.type == 'selectproduct'}
									<div class="row">
										<div class="col-lg-5">
											<input type="text" value="" id="product_name" name="product_name" placeholder="Search products"/>
