...
class AdminPosmegamenuController extends ModuleAdminController {
    public function __construct() {
        ...
        $this->display = 'view';
        parent::__construct();
        $this->meta_title = $this->trans('Pos Megamenu', array(), 'Modules.Linklist.Admin');
        
        if (!$this->module->active) {
            Tools::redirectAdmin($this->context->link->getAdminLink('AdminHome'));
        }
        if(!Tools::getValue('ajax')){
            Tools::redirectAdmin($this->context->link->getAdminLink('AdminModules').'&configure=posmegamenu');
        }
        ...
        foreach ($data as $param) {
            ...
            {   
                if ($param['name'] == 'customlink_title_'.$language['id_lang']) {
                    $model->customlink_title[$language['id_lang']] = $param['value'];
                }
