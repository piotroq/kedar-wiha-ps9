...
<div id="blockDisplay" class="panel panel-default col-lg-8 col-xs-10 col-lg-offset-2 col-xs-offset-1 inactive">
    <div class="panel-heading">
        {l s='Quickmenu item' d='Modules.Blockreassurance.Admin'}
    </div>
    ...
    {foreach from=$allblock item=$block key=$key}
        <form class="form_{$block['id_quickmenu']}" method="post" ENCTYPE="multipart/form-data">
            <div class="panel-body-{$block['id_quickmenu']} panel-body show-rea-block inactive">
                {* icon *}
                {include file="./config_elements/icon.tpl"}
                {* language *}
                {include file="./config_elements/language.tpl"}
