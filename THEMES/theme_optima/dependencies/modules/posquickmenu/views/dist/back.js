...
$(document).ready(function(){
	...
	$(document).on('click', '.psre-add', function () {
	  ...
	  $(document).on('change', 'select[name^="quickmenu_type_"]', (e) => {
	  ...
	  $(document).on('click', '#saveContentConfiguration', function () {
	    ...
	    formData.append('typelink', $('select[name="quickmenu_type_' + blockId + '"]').val());
	  ...
	  $(document).on('click', '#saveConfiguration', function () {
	    var color1 = $('#color_1').val(),
	    	color2 = $('#color_2').val(),
	    	show_text = $('input[name="QM_SHOW_TEXT"]:checked').val();
	    $.ajax({
	      type: 'POST',
	      dataType: 'JSON',
	      url: psr_controller_block_url,
