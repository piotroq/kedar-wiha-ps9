...
function upgrade_module_4_0_0($module)
{
    ...
    if (!empty($reassurances)) {
        foreach ($reassurances as $reassurance) {
            $sql[] = 'INSERT INTO ' . _DB_PREFIX_ . 'psreassurance (id_quickmenu, icon, custom_icon, status, position, id_shop, type_link, id_cms, date_add)
    ...
    if (!empty($reassurance_langs)) {
        foreach ($reassurance_langs as $reassurance_lang) {
            $sql[] = 'INSERT INTO ' . _DB_PREFIX_ . 'psreassurance_lang (id_quickmenu, id_lang, id_shop, title, description, link)
