...
class GetCarrierListHandler
{
    ...
    public function __construct(DpdshippingCarrierRepository $repository, $queryBus)
    {
        $this->repository = $repository;
        $this->queryBus = $queryBus;
    }

    public function handle(GetCarrierList $query)
    {
        $result = [];
        foreach (Shop::getContextListShopID() as $shop) {
            $result[] = $this->queryBus->handle(new GetCarrier($query->getType(), (int)$shop));
        }
