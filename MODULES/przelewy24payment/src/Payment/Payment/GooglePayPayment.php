...
class GooglePayPayment extends Przelewy24PaymentAbstract implements Przelewy24PaymentInterface
{
    ...
    {
        $paymentMethod = $this->config->getPayment()->getPaymentMethodNameList()->getPaymentMethodByType(PaymentTypeEnum::GOOGLE_PAYMENT);
    ...
    {
        if ($idPayment !== $this->getId() || !$this->config->getGoogle()->getOneClick()) {
            return false;
        }
    ...
    protected function buildPayload(): TransactionRegister
    {
        ...
        $xPayPayload->setType(XPayPayload::GOOGLE_PAY);
    ...
    {
        return new SessionIdConfig($this->params['id_payment'], $this->cart->id, SessionSpecialMethodTypeEnum::GOOGLE_PAY_IN_STORE);
    }
