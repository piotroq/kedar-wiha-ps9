...
class EtsonepagecheckoutSetMediaDriver implements ControllerSetMediaDriverInterface
{
    /**
     * @var \Context
     */
    ...
    {
        if (!class_exists(\Ets_onepagecheckoutOrderModuleFrontController::class)) {
            return [];
        }

        return [\Ets_onepagecheckoutOrderModuleFrontController::class];
    }
    ...
    public function registerMedia(Config $config, Przelewy24Config $accountConfig)
    {
        ...
        $this->context->controller->registerJavascript(
            $this->module->name . '-ets-onepagecheckout',
            'modules/' . $this->module->name . '/views/js/front/' . ModuleConfiguration::PRZELEWY24 . '-ets-onepagecheckout.js',
            [
                'position' => 'bottom',
            ]
        );
    ...
    public function modifyConfig(Config $config, Przelewy24Config $accountConfig)
    {
        ...
        $config->getOptions()->paymentDetails = [
            'total' => $this->context->cart->getOrderTotal() + $this->chargeCalculator->getAmount($accountConfig->getExtraCharge(), $this->context->cart),
