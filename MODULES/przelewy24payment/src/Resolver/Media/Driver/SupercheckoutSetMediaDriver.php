...
class SupercheckoutSetMediaDriver implements ControllerSetMediaDriverInterface
{
    /**
     * @var \Context
     */
    ...
    {
        if (!class_exists(\SupercheckoutSupercheckoutModuleFrontController::class)) {
            return [];
        }

        return [\SupercheckoutSupercheckoutModuleFrontController::class];
    }
    ...
    public function registerMedia(Config $config, Przelewy24Config $accountConfig)
    {
        ...
        $this->context->controller->registerJavascript(
            $this->module->name . '-super-checkout',
            'modules/' . $this->module->name . '/views/js/front/' . ModuleConfiguration::PRZELEWY24 . '-super-checkout.js',
            [
                'position' => 'bottom',
            ]
        );
