...
class Blik0TypeResolver implements PaymentTypeResolverInterface
{
    public function resolve(PaymentMethod $paymentMethod)
    {
        if ($paymentMethod->getId() == ModuleConfiguration::BLIK_LEVEL_O_ID_PAYMENT) {
            $paymentMethod->setType(PaymentTypeEnum::BLIK_LEVEL_O_PAYMENT);
        }
    }
}
