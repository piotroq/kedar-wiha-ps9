...
class InstallmentTypeResolver implements PaymentTypeResolverInterface
{
    public function resolve(PaymentMethod $paymentMethod)
    {
        if ($paymentMethod->getId() == ModuleConfiguration::CALCULATOR_ID_PAYMENT) {
            $paymentMethod->setType(PaymentTypeEnum::INSTALLMENTS_PAYMENT);
        }
    }
}
