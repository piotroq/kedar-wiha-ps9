...
class BlikType extends AbstractType
{
    ...
    {
        $builder
            ->setAction($this->router->generate('przelewy24.saveForm', ['type' => FormTypeEnum::BLIK]))
            ->add(
                'id_account', HiddenType::class, []
            )
            ->add(
                'blik_level_0', SwitchType::class, [
                    'label' => $this->translator->trans('Blik in store', [], 'Modules.Przelewy24payment.Form'),
                    'help' => $this->translator->trans('Blik in store', [], 'Modules.Przelewy24payment.Form'),
                ]
            );
    }
